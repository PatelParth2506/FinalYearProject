...
function Uploaded({ user }) {
    ...
    return (
        posts.map((post) => {
            ...
            return (
                <div className='w-full h-fit pt-5 px-0 font-sans sm:px-10'>
                    <div className="flex items-center justify-between px-5">
                        <div className="flex justify-start items-center gap-x-3 cursor-pointer">
                            <img src={post.owner.profilePhoto} alt="userPro" className='w-12 h-12 rounded-full object-cover' />
                            <div className="flex justify-center items-start flex-col">
                                <p className="text-[#2B6EA0] text-[16px] font-semibold">{post.owner.username}</p>
                                <p className="text-gray-500 text-sm font-sans">{user.fullname ?? "hello"}</p>
                            </div>
                            {friend && 
                    ...
                    <div className="w-full flex justify-between items-center mt-3 px-5">
                        <div className="flex gap-x-4">
                            <div className="flex items-center justify-center gap-x-2">
                                ...
                                <p className="text-sm font-sans text-gray-600">{post.likes.length} Likes</p>
                            </div>
                            <div className="flex items-center justify-center gap-x-2">
                                <img src={commentimg} alt="comments" className="cursor-pointer" onClick={() => setComment(pre => !pre)} />
                                <p className="text-sm font-sans text-gray-600">{post.comments.length}</p>
                            </div>
                            ...
                            <div className="w-full px-3 py-3 flex flex-col gap-y-3 bg-gray-100 rounded-xl">
                                <div className="w-full overFlow flex flex-col gap-y-1 max-h-40 overflow-y-auto ">
                                    ...
                                    {post.comments && post.comments.length > 0 ? (
                                        ...
                                        <p className="text-gray-500 text-sm font-sans">No comments yet.</p>
                                    )}
                                </div>
